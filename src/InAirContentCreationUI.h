...
#ifndef Carver_ThirdMenu_h
#define Carver_ThirdMenu_h
...
public:
    ...
    void updateOptions(float x, float y, float angle){
        ...
        // down->update(point3, point2, point2d, point3d,angle);
    ...
    float setup2(int numOptions){ 
        ...
        img.loadImage("images/tube.png");optionsSTL.push_back(new STLSubOptions(ofVec2f(x1,y1),40, "Tube",entryPoint, repo, "stls/tube.stl",ofColor(0,159,215),img));
        return 2;
    }
        ...
        if(isOnTable){
            if(visible){ 
                if(ofGetElapsedTimeMillis() - lastUsed < 6000){
                    ...
                    centerop->draw();left->draw(); right->draw(); up->draw();
    ...
    bool inside(float x, float y){
        if(visible){
            if(right->inside(x, y)){return true;
            }else if(left->inside(x, y)){return true;
            }else  if(centerop->inside(x, y)){return true;
            }else if(up->inside(x, y)){return true;
            }//else  if(down->inside(x, y)){return true;
             // }
        }
        else{
                ...
                switch(currentMenu){
                    ...
                    case COPY:  {
                        if(optionsCopy.size() == 0){
                            ...
                            if(optionCopy == (*optionsCopy.begin())){
        ...
        switch(fc){
            ...
            case InAirOption::MAINMENUENTER:{
                switch (mainMenuOptionNumber) {
                    ...
                    case 1:
                    {   
                        currentMenu = KINECT; assignFunctions();
                        return RETURNKINECT;
                        break;
                    }  
                    case 2:

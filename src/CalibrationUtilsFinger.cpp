...
//--------------------------------------------------------------
void CalibrationUtilsFinger::loadXMLSettings(string afilename)
{
	bGoToNextStep = false;
...
//Bounding Box Size
void CalibrationUtilsFinger::setScreenBBox(rect2df &box)
{
	screenBB = box;
	initScreenPoints();
}
...
void CalibrationUtilsFinger::computeCameraToScreenMap()
{
...
void CalibrationUtilsFinger::setGrid(int x, int y)
{
	GRID_Y = y;
	GRID_X = x;
...
void CalibrationUtilsFinger::setCamRes(int camWidth = 320, int camHeight = 240)
{
	_camWidth = camWidth;
	_camHeight = camHeight;
}

void CalibrationUtilsFinger::initTriangles()
{
	int t = 0;
...
//Initialize Points
void CalibrationUtilsFinger::initScreenPoints()
{
	int p = 0;
...
void CalibrationUtilsFinger::initCameraPoints(int camWidth, int camHeight)
{
	int p = 0;
	for(int j = 0; j <= GRID_Y; j++)
	{
...
void CalibrationUtilsFinger::setScreenScale(float s)
{
	// legacy
...
float CalibrationUtilsFinger::getScreenScale()
{
...
void CalibrationUtilsFinger::calculateBox()
{
	//reset variables
	maxBoxX = 0;
	minBoxX = _camWidth;
...
std::pair<ofVec2f, bool >  CalibrationUtilsFinger::cameraToScreenSpace(float x, float y)
{
	vector2df pt(x, y);
...
bool CalibrationUtilsFinger::isPointInTriangle(vector2df p, vector2df a, vector2df b, vector2df c)
{
...
int CalibrationUtilsFinger::findTriangleWithin(vector2df pt)
{
...
void CalibrationUtilsFinger::beginCalibration()
{
	 bCalibrating = true;
	 calibrationStep = 0;
}

void CalibrationUtilsFinger::nextCalibrationStep()
{
	if(bCalibrating)
	{
		calibrationStep++;
...
void CalibrationUtilsFinger::revertCalibrationStep()
{
	if(bCalibrating)
	{
		calibrationStep--;
...
void CalibrationUtilsFinger::saveCalibration()
{
	// -------------------------------- SAVE STATE ON EXIT

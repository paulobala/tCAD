...
//  Carver
//
//  Created by paulobala on 03/09/12.
//  Copyright (c) 2012 __MyCompanyName__. All rights reserved.
//

#ifndef Carver_SaveToken_h
#define Carver_SaveToken_h
...
public:
    ...
    void action(){
        ...
        for (std::vector<Shape3D*>::iterator it=temp.begin() ; it < temp.end(); it++ ){
            std::vector<ofxSTLFacet>* facets2 = new std::vector<ofxSTLFacet>();
            (*it)->getFacets(facets2);
            
            for (std::vector<ofxSTLFacet>::iterator it2 = facets2->begin() ; it2 < facets2->end(); it2++ ){

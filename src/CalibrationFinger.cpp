 ...
 *****************************************************************************/
void CalibrationFinger::setup(int _camWidth, int _camHeight)//, BlobTracker *trackerIn)
{
	/********************
	 * Initalize Variables
	 *********************/
	...
	calibrate.loadXMLSettings("calibration.xml");
 ...
 *		  CALIBRATION
 *******************************/
void CalibrationFinger::doCalibration()
{
...
void CalibrationFinger::drawCalibrationBlobs()
{
}
...
void CalibrationFinger::touchUp(Finger * touch){
    bool found = false;
    for (vector<pair <Finger *, unsigned long> >::iterator it= touches.begin() ; it < touches.end() && !found; it++ ){
        if((*it).first->getFingerID()==touch->getFingerID()){
            found = true;
            touches.erase(it);
        }
    }
}
void CalibrationFinger::update(){
    for (vector<pair <Finger *, unsigned long> >::iterator it= touches.begin() ; it < touches.end(); it++ ){
        if((*it).second == 0){//has been held long enough
            if(calibrate.bCalibrating)//If Calibrating, register the calibration point on blobOff
            {
                if(calibrate.bGoToNextStep) {
                    ...
                    if(calibrate.calibrationStep != 0){
                        printf("%d (%f, %f)\n", calibrate.calibrationStep, (*it).first->getX()* 640 , (*it).first->getY()*480);
                    }
                    else{
                        printf("%d (%f, %f)\n", calibrate.GRID_POINTS,(*it).first->getX() * 640, (*it).first->getY()*480);
            ...
            if((*it).second != 0){
                ...
                if( currenttime - (*it).second > 1000){
                    ...
                    calibrate.cameraPoints[calibrate.calibrationStep] = vector2df((*it).first->getX() * 640, (*it).first->getY()*480);
 ...
 * UTILS
 ******************************************************************************/
void CalibrationFinger::DrawCircleLoader(double xctr, double yctr, double radius, double startAngle, double endAngle)
{
 ...
 * KEY EVENTS
 *****************************************************************************/
void CalibrationFinger::_keyPressed(ofKeyEventArgs &e) {
...
bool CalibrationFinger::insideCalibrationRectangle(float x, float y){
    ofRectangle rect = ofRectangle(calibrate.screenBB.upperLeftCorner.X * ofGetWidth(), calibrate.screenBB.upperLeftCorner.Y * ofGetHeight(),
                                   calibrate.screenBB.getWidth() * ofGetWidth(), calibrate.screenBB.getHeight() * ofGetHeight());
    return rect.inside(ofVec2f(x,y));
                       
                       }
                       
void CalibrationFinger::_keyReleased(ofKeyEventArgs &e){

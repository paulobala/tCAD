...
#include <cmath>
CalibrationUtilsMarker::CalibrationUtilsMarker()
{   //Default values
	_camWidth = 320;
	_camHeight = 240;
    filename = "calibrationMarker.xml";
}
...
//--------------------------------------------------------------
void CalibrationUtilsMarker::loadXMLSettings(string afilename)
{
	bGoToNextStep = false;

	// Can this load via http?
	if( calibrationXML.loadFile(filename))
		//WOOT!
		...
		if(numDragTags > 0)
		{
			...
			if(numPtTags > 0)
			{
				...
				for(int i = 0; i < numPtTags; i++)
				{
					...
					float x = calibrationXML.getValue("POINT:X", 0.000000, i);
					float y = calibrationXML.getValue("POINT:Y", 0.000000, i);
...
//Bounding Box Size
void CalibrationUtilsMarker::setScreenBBox(rect2df &box)
{
	screenBB = box;
	initScreenPoints();
}
...
void CalibrationUtilsMarker::computeCameraToScreenMap()
{
...
void CalibrationUtilsMarker::setGrid(int x, int y)
{
	GRID_Y = y;
	GRID_X = x;
...
void CalibrationUtilsMarker::setCamRes(int camWidth = 320, int camHeight = 240)
{
	_camWidth = camWidth;
	_camHeight = camHeight;
}

void CalibrationUtilsMarker::initTriangles()
{
	int t = 0;
...
//Initialize Points
void CalibrationUtilsMarker::initScreenPoints()
{
	int p = 0;
...
void CalibrationUtilsMarker::initCameraPoints(int camWidth, int camHeight)
{
	int p = 0;
	for(int j = 0; j <= GRID_Y; j++)
	{
...
void CalibrationUtilsMarker::setScreenScale(float s)
{
	// legacy
...
float CalibrationUtilsMarker::getScreenScale()
{
...
std::pair<ofVec2f, bool >  CalibrationUtilsMarker::cameraToScreenPosition(float x, float y)
{
...
void CalibrationUtilsMarker::transformDimension(float &width, float &height)
{
...
void CalibrationUtilsMarker::calculateBox()
{
	//reset variables
	maxBoxX = 0;
	minBoxX = _camWidth;
...
std::pair<ofVec2f, bool >  CalibrationUtilsMarker::cameraToScreenSpace(float x, float y)
{
	vector2df pt(x, y);
	...
	if(t != -1)
	{
		...
		float bary_A = area_A / (float)total_area;
		float bary_B = area_B / (float)total_area;
		...
		float ax = transformedPos.X;
		float ay = transformedPos.Y;
        return std::pair<ofVec2f, bool > (ofVec2f(ax,ay), true);
	}
	float ax = 0;
	float ay = 0;
    return pair< ofVec2f, bool > (ofVec2f(ax,ay), false);
    
	// FIXME: what to do in the case that it's outside the mesh?
}

bool CalibrationUtilsMarker::isPointInTriangle(vector2df p, vector2df a, vector2df b, vector2df c)
{
...
int CalibrationUtilsMarker::findTriangleWithin(vector2df pt)
{
...
void CalibrationUtilsMarker::beginCalibration()
{
	 bCalibrating = true;
	 calibrationStep = 0;
}

void CalibrationUtilsMarker::nextCalibrationStep()
{
	if(bCalibrating)
	{
		calibrationStep++;
...
void CalibrationUtilsMarker::revertCalibrationStep()
{
	if(bCalibrating)
	{
		calibrationStep--;
...
void CalibrationUtilsMarker::saveCalibration()
{
	// -------------------------------- SAVE STATE ON EXIT
	...
	calibrationXML.saveFile(filename);
}

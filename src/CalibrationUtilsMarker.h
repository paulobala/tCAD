...
class CalibrationUtilsMarker
{
	public:
        ...
        string filename;
        CalibrationUtilsMarker();
		~CalibrationUtilsMarker();//destructor

		void loadXMLSettings(string afilename);// Load Settings
		...
		std::pair<ofVec2f, bool >  cameraToScreenPosition(float x, float y);
		std::pair<ofVec2f, bool >  cameraToScreenSpace(float x, float y);

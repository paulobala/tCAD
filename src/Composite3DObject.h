#ifndef tCAD_Composite3DObject_h
#define tCAD_Composite3DObject_h
#include "Shape3D.h"
#include "ofxCarve.h"
#include "Base3DObject.h"

/*
 Object used for Boolean operations
 */
...
public:
    ...
    /*
     Convert children into Meshset for ofxCarveCSG
     */
    ...
    /*
     Convert Meshset from ofxCarveCSG into a ofMeshtCAD
     */
    ...
    /*
     Do all boolean operations
     */
    void allOperations()
    {
        carve::mesh::MeshSet<3> * objA = readChildren(children[0]);
        carve::mesh::MeshSet<3> * objB = readChildren(children[1]);
        ofxCarveCSG carver = ofxCarveCSG();
        std::vector<std::pair<carve::mesh::MeshSet<3> *, ofxCarveCSG::OP > > results = std::vector<std::pair<carve::mesh::MeshSet<3> *, ofxCarveCSG::OP > > ();
        ...
        {
            hasShape = true;
            for (std::vector<std::pair<carve::mesh::MeshSet<3> *, ofxCarveCSG::OP > >::iterator it =results.begin() ; it < results.end(); it++ )
            {
                switch((*it).second)
                {
                    case ofxCarveCSG::UNION:
                        if((*it).first != NULL)
                        {
                            mesh.clear();
                    ...
                    case ofxCarveCSG::B_MINUS_A:
                        ...
                        break;
                    case ofxCarveCSG::SYMMETRIC_DIFFERENCE:
                        if((*it).first != NULL)
                        {
                            mesh.clear();
                            meshSetToMesh((*it).first);
        ...
        if(hasShape)
        {
            switch(bop)
            {
                ...
                case BOOLEAN_DIFFERENCE_A_MINUS_B:
                    mesh = meshDifferenceAB;
                    hasChosenMesh = true;
                    break;
                case BOOLEAN_DIFFERENCE_B_MINUS_A:
                    mesh = meshDifferenceBA;
                    hasChosenMesh = true;
                    break;
    ...
    /*
     Gets triangles for STL exporter
     */
